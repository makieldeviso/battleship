...
const shipMovementHandler = function () {
  let shipUnit = null;
  let hoveredCells = [];

  const setShipUnit = function (ship) {
    shipUnit = ship;
  }

  const getShipObj = function () {
    const currentPlayerShips = memory.current.player.gameBoard.ships;
    const shipObj = currentPlayerShips.find(ship => ship.name === shipUnit.title);
    return shipObj;
  }

  const getCurrentShipPlacement = function () {
    const shipObj = getShipObj();
    return shipObj.placement;
  }

  const getCurrentDomPlacement = function () {
    const shipObj = getShipObj();
    const currentDomPlacement = shipObj.placement.map(coor => document.querySelector(`div.cell[data-column='${coor[0]}'][data-row='${coor[1]}']`));
    return currentDomPlacement;
  }

  const getInitShipPos = function () {
    const initShipPosX = shipUnit.offsetLeft;
    const initHipPosY = shipUnit.offsetTop;
    return {x: initShipPosX, y:initHipPosY};
  }

  return {shipUnit, hoveredCells, setShipUnit, getShipObj, getCurrentShipPlacement, getCurrentDomPlacement}
} 
...
const selectShip = function (event) {
  ...
  if (event.button === 0) {
    ...
    const initShipPosX = shipUnit.offsetLeft;
    const initHipPosY = shipUnit.offsetTop;
    let hoveredCells = []; // Contains nodes of cells the ship is currently hovering

    // Remove the shipObj from the gameBoard obj
    shipObj.removePlace();

    // Move the ship unit along the cursor
    // Note: event.clientX and Y is equal to position of cursor when ship was clicked
    ...
    const placeShipUnit = function (event) {
      ...
      const hoveredCellsAreValid = !hoveredCells.includes(null) && !hoveredCells.includes(undefined) && hoveredCells.length !== 0;
      ...
      if (hoveredCellsAreValid && event.button !== 2) {
        ...
        shipUnit.style.left = `${initShipPosX}px`;
        shipUnit.style.top = `${initHipPosY}px`;
...
const createShipUnit = function (shipObj, playerName) {
  ...
  if (playerName === 'player') {
    ...
    shipWhole.addEventListener('dblclick', rotateShip);
  }
}
